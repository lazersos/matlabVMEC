...
disp(['  Constructing : ' num2str(length(xs)*length(ys)*length(zs),'%i') ' blocks.']);
...
% Create x1x2x3 style arrays
x1x2x3=zeros(3,wall.nfaces);
y1y2z3=zeros(3,wall.nfaces);
z1z2z3=zeros(3,wall.nfaces);
d1 = wall.faces(1,:);
d2 = wall.faces(2,:);
d3 = wall.faces(3,:);
x1x2x3 = [wall.coords(1,d1); wall.coords(1,d2); wall.coords(1,d3)];
y1y2y3 = [wall.coords(2,d1); wall.coords(2,d2); wall.coords(2,d3)];
z1z2z3 = [wall.coords(3,d1); wall.coords(3,d2); wall.coords(3,d3)];

% assign triangles to grid
for i=1:length(grid_blocks)
    disp(['   -- Working on block : ' num2str(i,'%i')]);
    tmp = grid_blocks(i);
    ...
    %check which verticies are in this block
    l1 = sum(and(x1x2x3<xmax,x1x2x3>=xmin),1)>0;
    l2 = sum(and(y1y2y3<ymax,y1y2y3>=ymin),1)>0;
    l3 = sum(and(z1z2z3<zmax,z1z2z3>=zmin),1)>0;
    mask = and(and(l1,l2),l3);
    
    % Get faces
    dex = find(mask);
    
    % store the unique faces
    if ~isempty(dex)
        grid_blocks(i).faces = dex;
        grid_blocks(i).nfaces = length(dex);
        disp(['        Found : ' num2str(grid_blocks(i).nfaces,'%i') ' faces']);
    end
...
wall_accelerated.machine = wall.machine;
    wall_accelerated.date=datestr(now,'mm-dd-yy');
