function f=sfunct_old(theta,zeta,zbs,nfp)
...
% the transform where:
% f=\sum_{m=0,M}\sum_{n=-N,N} zbs(m,n)*sin(m*theta-n*nfp*zeta)
% the VMEC Fourier transform.
%
% f=SFUNCT_OLD(theta,zeta,zbc,nfp)
% Inputs
...
%      z=sfunct_old(theta,zeta,data.zbs,data.nfp);
%
...
for m=1:index1
    for n=1:index2
        for i=1:index3
            ...
            mdex=m-1;
            f(:,1:zbound,i)=f(:,1:zbound,i)+zbs(m,n,i).*sin(mdex.*theta(:,1:zbound,i)+ndex.*zeta(:,1:zbound,i));
        end
    end
end
