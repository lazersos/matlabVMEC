function f=cnfunct(theta,zeta,rbc,nfp)
% CNFUNCT(theta,zeta,rbc,nfp) Cosine Fourier Transform for derivatives.
...
% f=\sum_{m=0,M}\sum_{n=-N,N} rbc(m,n)*n*nfp*cos(m*theta-n*nfp*zeta)
% the VMEC Fourier transform.
%
% f=CNFUNCT(theta,zeta,zbc,nfp)
% Inputs
...
%      r=cnfunct(theta,zeta,data.rbc,data.nfp);
%
...
for m=1:index1
    for n=1:index2
        for i=1:index3
            ...
            mdex=m-1;
            f(:,1:zbound,i)=f(:,1:zbound,i)-rbc(m,n,i).*ndex.*cos(mdex.*theta(:,1:zbound,i)-ndex.*zeta(:,1:zbound,i));
        end
    end
end
