function [rho, pell, pill] = beams3d_calc_pll(beam_data,vmec_data)
%BEAMS3D_CALC_PLL Calcluate parallel momentum profile from distribution
%   The BEAMS3D_CALC_PLL function calculats the parallel momentum
%   depoistion from the 5D BEAMS3D distribution function.  It takes a
%   BEAMS3D data structure (as read from READ_BEAMS3D) and a VMEC data
%   strucutre (as read from READ_VMEC) as inputs.  The radial grid (rho), 
%   depositied parallel electron momentum (pell, [kg*m/s^2]), and the
%   depositied parallel ion momentum (pill [kg*m/s^2]) are returned. The 
%   pell and pill variables are [nbeams,nrho] sized.
%
%   Example usage
%      vmec_data = read_vmec('wout_test.nc');
%      beam_data = read_beams3d('beams3d_test.h5');
%      [rho,pell,pill]=beams3d_calc_pll(beam_data,vmec_data);
%
%   See also read_beams3d, read_vmec, beams3d_calc_power.
%
...
% Now the change in momentum is dp=m*dv*pitch pitch=vll/vtot
dpell = zeros(nbeams,nrho,npol,ntor,nvpa,nvpe);
dpill = zeros(nbeams,nrho,npol,ntor,nvpa,nvpe);
for i=1:nvpa
    for j=1:nvpe
        dpell(:,:,:,:,i,j) = dve(:,:,:,:,i,j).*pitch_grid(i,j);
        dpill(:,:,:,:,i,j) = dvi(:,:,:,:,i,j).*pitch_grid(i,j);
    end
end
pell = squeeze(sum(dpell.*beam_data.dist_prof,[3 4 5 6]));
pill = squeeze(sum(dpill.*beam_data.dist_prof,[3 4 5 6]));
for i=1:nbeams
    pell(i,:) = pell(i,:) .*mass(i);
    pill(i,:) = pill(i,:) .*mass(i);
end
